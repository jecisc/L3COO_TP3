...
public class MailSimulation {
	...
	private static Letter<Content> Decoration(Letter<Content> letter) {
		Random randLetterDeco = new Random();
		Letter<Content> letterFinal = letter;
		int randomInt = randLetterDeco.nextInt(4);
		if (randomInt == 0){
			letterFinal = new RegisteredLetter(letter);
			}
		
		randomInt = randLetterDeco.nextInt(4);
		
		if (randomInt == 0){
			letterFinal = new UrgentLetter(letterFinal);
			}
	...
	public static void main(String[] args) {
		...
		for(int Day=1;Day <= numberDays; Day++){
			...
			int numberActions = rand.nextInt(3)+1;
